...
const STYLE_CLASSES = ['quasar-style-md2', 'quasar-style-md3', 'quasar-style-unstyled']
...
onMounted(() => {
  ...
  const cls = style === 'md2' ? 'quasar-style-md2' : style === 'unstyled' ? 'quasar-style-unstyled' : 'quasar-style-md3'
  for (const c of STYLE_CLASSES) document.body.classList.toggle(c, c === cls)
  // Set dark mode
  if (new URLSearchParams(window.location.search).get('dark') === 'true') $q.dark.set(true)
})
