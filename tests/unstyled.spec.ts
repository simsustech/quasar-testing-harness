...
test.describe('Unstyled style', () => {
  ...
  test('QBtn has no background or primary color by default', async ({
    page
  }) => {
    await page.setViewportSize({ width: 1024, height: 768 })
    await page.goto('/q-btn?style=unstyled', {
  ...
  test('Binding a Quasar color prop does not theme the component', async ({
    page
  }) => {
    await page.setViewportSize({ width: 1024, height: 768 })
    await page.goto('/q-btn?style=unstyled&color=secondary', {
      waitUntil: 'networkidle',
      timeout: 20000
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
    ...
    await page.waitForSelector('.q-btn', { timeout: 5000 })
    const bg = await page.evaluate(
      () => getComputedStyle(document.querySelector('.q-btn')!).backgroundColor
    )
    // Should be transparent — the color prop should NOT force a background in unstyled
    expect(bg).toBe('rgba(0, 0, 0, 0)')
  })

  test('Rendering multiple components with no theme leaking', async ({
    page
  }) => {
    await page.setViewportSize({ width: 1024, height: 768 })
    await page.goto('/q-card?style=unstyled', {
      waitUntil: 'networkidle',
      timeout: 20000
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
