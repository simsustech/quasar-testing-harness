...
test.describe('QToggle — functional', () => {
  test('toggles on click and updates the URL', async ({ page }) => {
    await page.goto('/q-toggle?label=Test&modelValue=true', {
  ...
  test('control panel modelValue input drives the toggle', async ({ page }) => {
    await page.goto('/q-toggle?label=Test&modelValue=false', {
      waitUntil: 'networkidle'
    })
    await expect(page.locator('.control-panel')).toBeVisible({
      timeout: 10_000
...
test.describe('QCheckbox — functional', () => {
  test('toggles on click and updates the URL', async ({ page }) => {
    await page.goto('/q-checkbox?label=Test&modelValue=true', {
      waitUntil: 'networkidle'
    })
    await expect(page.locator('.control-panel')).toBeVisible({
      timeout: 10_000
...
test.describe('QBtn — functional', () => {
  test('button renders with given label', async ({ page }) => {
    await page.goto('/q-btn?label=ClickMe&color=primary', {
      waitUntil: 'networkidle'
    })
    await expect(page.locator('.control-panel')).toBeVisible({
      timeout: 10_000
  ...
  test('disabled button is not clickable', async ({ page }) => {
    await page.goto('/q-btn?label=Off&disable=true&color=primary', {
      waitUntil: 'networkidle'
    })
    const btn = page
      .locator('[data-testid="component-preview"]')
...
test.describe('QDialog — functional', () => {
  test('dialog is visible by default and closes with Cancel', async ({
    page
  }) => {
...
test.describe('QSelect — functional', () => {
  test('select opens dropdown with options', async ({ page }) => {
    await page.goto('/q-select?modelValue=de&label=Country&style=unstyled', {
      waitUntil: 'networkidle'
    })
    await expect(page.locator('.control-panel')).toBeVisible({
      timeout: 10_000
