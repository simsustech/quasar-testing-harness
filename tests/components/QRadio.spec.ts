...
test.describe('QRadio — prop variations', () => {
  const variants = [
    {
      label: 'default',
      query: 'label=Express+shipping&modelValue=express&val=express'
    },
    {
      label: 'color-secondary',
      query: 'label=Secondary&color=secondary&modelValue=opt&val=opt'
    },
    {
      label: 'dense',
      query: 'label=Compact&dense=true&modelValue=opt&val=opt'
    },
    {
      label: 'left-label',
      query: 'label=Left+side&leftLabel=true&modelValue=opt&val=opt'
    },
    {
      label: 'keep-color',
      query: 'label=Teal&keepColor=true&color=teal&modelValue=opt&val=opt'
    },
    {
      label: 'disabled',
      query: 'label=Disabled&disable=true&modelValue=opt&val=opt'
    }
  ]
...
test.describe('QRadio — MD3 spec conformance', () => {
  test('checked radio has on-primary background and primary border', async ({
    page
  }) => {
    await page.goto(
      `/${SLUG}?style=md3&label=Checked&modelValue=opt&val=opt&color=primary`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    ...
    const inner = page.locator(
      '[data-testid="component-preview"] .q-radio__inner--truthy'
    )
  ...
  test('dense — font-size is smaller', async ({ page }) => {
    await page.goto(
      `/${SLUG}?style=md3&dense=true&label=Compact&modelValue=opt&val=opt`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
