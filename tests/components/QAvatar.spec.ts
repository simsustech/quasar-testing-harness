import { test, expect } from '@playwright/test'
import fs from 'node:fs'

import { styleFromPage, shotPath, shot, dumpDiagnostics } from '../helpers.js'

const SLUG = 'q-avatar'
const STYLES = ['md3', 'md2', 'unstyled'] as const

test.describe('QAvatar', () => {
  for (const style of STYLES) {
    test(`renders cleanly with ?style=${style}`, async ({ page }) => {
      await page.goto(`/${SLUG}?style=${style}`, { waitUntil: 'networkidle' })
      await expect(page.locator('.control-panel')).toBeVisible({
        timeout: 10_000
      })
      await expect(page.getByTestId('component-preview')).toBeVisible({
        timeout: 30_000
      })
      const png = await shot(page, SLUG, { style }, style)
      await dumpDiagnostics(page, SLUG, { style }, style)
      expect(fs.existsSync(png)).toBe(true)
      expect(fs.statSync(png).size).toBeGreaterThan(100)
    })
  }
})

test.describe('QAvatar — prop variations', () => {
  const variants = [
    { label: 'default', query: 'color=primary&icon=i-mdi-android&size=48px' },
    { label: 'rounded', query: 'color=teal&icon=i-mdi-account&rounded=true' },
    { label: 'square', query: 'color=orange&icon=i-mdi-fire&square=true' },
    {
      label: 'text-color',
      query: 'color=purple&textColor=white&icon=i-mdi-star'
    },
    {
      label: 'color-secondary',
      query: 'color=secondary&textColor=white&label=JD'
    },
    { label: 'size-lg', query: 'color=primary&icon=i-mdi-camera&size=72px' },
    { label: 'size-xs', query: 'color=primary&icon=i-mdi-camera&size=xs' },
    { label: 'size-sm', query: 'color=primary&icon=i-mdi-camera&size=sm' },
    { label: 'size-xl', query: 'color=primary&icon=i-mdi-camera&size=xl' },
    {
      label: 'font-size-lg',
      query: 'color=primary&textColor=white&label=JD&fontSize=32px'
    }
  ]

  test('all visual variants at md3', async ({ page }) => {
    for (const v of variants) {
      await page.goto(`/${SLUG}?style=md3&${v.query}`, {
        waitUntil: 'networkidle'
      })
      await expect(page.getByTestId('component-preview')).toBeVisible({
        timeout: 30_000
      })
      const png = await shot(page, SLUG, v.label, 'md3')
      await dumpDiagnostics(page, SLUG, v.label, 'md3')
      expect(fs.statSync(png).size).toBeGreaterThan(100)
    }
  })
})

test.describe('q-avatar — dark mode', () => {
  test('renders cleanly with ?style=md3&dark=true', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&dark=true`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    })
    await shot(page, SLUG, 'dark', 'md3')
    await dumpDiagnostics(page, SLUG, 'dark', 'md3')
  })
})
