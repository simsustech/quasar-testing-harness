...
test.describe('QPage — padding prop', () => {
  for (const style of PADDING_STYLES) {
    test(`padding=true adds q-layout-padding class and responsive padding (?style=${style})`, async ({
      page
    }) => {
      await page.goto(`/${SLUG}?style=${style}&padding=true`, {
      ...
      const qpage = page.locator(
        '[data-testid="component-preview"] main.q-page'
      )
    ...
    test(`padding=false does NOT add q-layout-padding (?style=${style})`, async ({
      page
    }) => {
      await page.goto(`/${SLUG}?style=${style}`, { waitUntil: 'networkidle' })
      await expect(page.locator('.control-panel')).toBeVisible({
