...
test.describe('QInput — dark mode and clearable', () => {
  const variants = [
    ...
    {
      label: 'filled-dark',
      query: 'filled=true&dark=true&label=Email&modelValue=test@example.com'
    },
    {
      label: 'outlined-dark',
      query: 'outlined=true&dark=true&label=Email&modelValue=test@example.com'
    },
    {
      label: 'clearable',
    ...
    },
    {
      ...
      query: 'dark=true&clearable=true&label=Email&modelValue=test@example.com'
    }
  ]
...
test.describe('QInput — MD3 spec conformance', () => {
  ...
  test('dark mode — native text is NOT black', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&dark=true&label=Email`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
  ...
  test('clearable — clear icon is present when modelValue set', async ({
    page
  }) => {
    await page.goto(
      `/${SLUG}?style=md3&clearable=true&label=Email&modelValue=test@example.com`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    ...
    const icon = page.locator(
      '[data-testid="component-preview"] .q-field__append .q-icon'
    )
