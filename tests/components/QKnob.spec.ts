...
test.describe('QKnob — prop variations', () => {
  const variants = [
    ...
    { label: 'color-secondary', query: 'modelValue=65&showValue=true&color=secondary' },
...
test.describe('QKnob — MD3 spec conformance', () => {
  test('knob svg circle has stroke matching the color', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=65&showValue=true`, { waitUntil: 'networkidle' })
    await expect(page.getByTestId('component-preview')).toBeVisible({ timeout: 10_000 })
  ...
  test('disabled knob has reduced opacity', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=65&showValue=true&disable=true`, { waitUntil: 'networkidle' })
    await expect(page.getByTestId('component-preview')).toBeVisible({ timeout: 10_000 })
