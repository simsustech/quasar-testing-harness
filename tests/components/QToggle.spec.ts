...
test.describe('QToggle — prop variations', () => {
  const variants = [
    ...
    { label: 'dense', query: 'label=Compact&dense=true&modelValue=true' },
    {
      ...
      query: 'label=Compact&dense=true&modelValue=false'
    },
...
test.describe('QToggle — MD3 spec conformance', () => {
  test('default (unchecked) — toggle renders with reasonable dimensions', async ({
    page
  }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=false&label=Off`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    ...
    expect(parseFloat(s['.q-toggle__thumb']?.['left'] ?? '0')).toBeGreaterThan(
      0
    )
    expect(parseFloat(s['.q-toggle__thumb']?.['width'] ?? '0')).toBeGreaterThan(
      0
    )
    expect(
      parseFloat(s['.q-toggle__thumb']?.['height'] ?? '0')
    ).toBeGreaterThan(0)
  })

  test('checked — thumb moves to the right', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=true&label=On`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
  ...
  test('label has non-zero left padding', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=true&label=Notifications`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    ...
    const pad = await page.evaluate(() => {
      const el = document.querySelector(
        '[data-testid="component-preview"] .q-toggle .q-toggle__label'
      ) as HTMLElement | null
      if (!el) return null
  ...
  test('hover glow — ::before has an opacity value', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=false&label=Hover`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    ...
    const thumb = page.locator(
      '[data-testid="component-preview"] .q-toggle__thumb'
    )
    await thumb.hover()
  ...
  test('dense — font-size 28px', async ({ page }) => {
    await page.goto(
      `/${SLUG}?style=md3&dense=true&modelValue=true&label=Compact`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
...
test.describe('QToggle — MD2 spec conformance', () => {
  ...
  test('checked — thumb left > off-state position', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md2&modelValue=true&label=On`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
  ...
  test('dense — inner width smaller than default', async ({ page }) => {
    await page.goto(
      `/${SLUG}?style=md2&dense=true&modelValue=false&label=Compact`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
  ...
  test('hover glow — ::before becomes visible on hover', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md2&modelValue=false&label=Hover`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
