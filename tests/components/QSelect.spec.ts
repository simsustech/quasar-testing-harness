...
test.describe('QSelect — prop variations', () => {
  const variants = [
    ...
    { label: 'dark', query: 'modelValue=de&label=Country&dark=true' },
    {
      ...
      query: 'modelValue=de&label=Country&dark=true&clearable=true'
    }
  ]
...
test.describe('QSelect — MD3 spec conformance', () => {
  test('clearable — native has pr-48px to make room for clear and dropdown icons', async ({
    page
  }) => {
    await page.goto(
      `/${SLUG}?style=md3&modelValue=de&label=Country&clearable=true`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
  ...
  test('with value — dropdown icon is present', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&modelValue=de&label=Country`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
    ...
    const icon = page.locator(
      '[data-testid="component-preview"] .q-select__dropdown-icon'
    )
...
test.describe('QSelect — MD2 spec conformance', () => {
  test('clearable — native has pr-48px', async ({ page }) => {
    await page.goto(
      `/${SLUG}?style=md2&modelValue=de&label=Country&clearable=true`,
      { waitUntil: 'networkidle' }
    )
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
