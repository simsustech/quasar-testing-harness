...
test.describe('QBadge — prop variations', () => {
  const variants = [
    ...
    {
      label: 'rounded',
      query: 'label=999+&color=primary&rounded=true&floating=true'
    },
    {
      label: 'transparent',
      query: 'label=%E2%88%9E&color=orange&transparent=true'
    },
    {
      label: 'color-secondary',
      query: 'label=New&color=secondary&floating=true'
    },
    ...
    {
      label: 'multi-line',
      query: 'label=Long+label+text&color=primary&multiLine=true'
    },
...
test.describe('QBadge — MD3 spec conformance', () => {
  test('badge has primary background color', async ({ page }) => {
    await page.goto(`/${SLUG}?style=md3&label=3&color=primary&floating=true`, {
      waitUntil: 'networkidle'
    })
    await expect(page.getByTestId('component-preview')).toBeVisible({
      timeout: 30_000
