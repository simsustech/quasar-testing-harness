...
function slugify(name) {
  return 'q-' + name.replace(/^Q/, '').replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()
}
...
function pascalToKebab(str) {
  return str.replace(/^Q/, '').replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()
}
...
function readSourceProps() {
  ...
  const sourceProps = {}
  for (const dir of readdirSync(QUASAR_UI, { withFileTypes: true }).filter(d => d.isDirectory())) {
    const slug = slugify(dir.name)
    const pascalName = dir.name.charAt(0).toUpperCase() + dir.name.slice(1).replace(/-([a-z])/g, (_,c) => c.toUpperCase())
    ...
    if (existsSync(jsonFile)) {
      try {
        ...
        sourceProps[slug] = {
          ...
          categories: [...new Set(Object.values(props).map(p => p.category || 'other'))],
        }
        continue
      } catch { /* fall through */ }
    }
    ...
    if (srcFile) {
      ...
      const propNames = [...content.matchAll(/['"](\w+)['"]\s*:\s*\{/g)]
        .map(m => m[1])
        .filter(n => n !== 'props' && !['type', 'default', 'required', 'validator'].includes(n))
      sourceProps[slug] = {
...
function readSpecs() {
  ...
  for (const f of readdirSync(specsDir).filter(f => f.endsWith('.spec.ts'))) {
    ...
    const labels = []
    const variantMatch = content.match(/const\s+variants\s*=\s*\[([\s\S]*?)\]\s*\n\s*\n\s*test/)
    if (variantMatch) {
    ...
    const conformance = []
    const describeBlocks = content.matchAll(/test\.describe\('([^']+)(?:conformance|spec conformance)([^']*)'/gi)
    for (const m of describeBlocks) {
      const blockName = m[0]
      const tests = [...content.slice(content.indexOf(blockName)).matchAll(/test\('([^']+)'/g)].map(t => t[1])
...
function readDocExamples() {
  ...
  const docMap = {}
  for (const dir of readdirSync(DOCS_EXAMPLES, { withFileTypes: true }).filter(d => d.isDirectory())) {
    const slug = slugify(dir.name)
    const examples = []
    for (const f of readdirSync(join(DOCS_EXAMPLES, dir.name)).filter(f => f.endsWith('.vue'))) {
      ...
      for (const tm of template.matchAll(tagPattern)) {
        const attrs = tm[0].match(new RegExp(`<${slug}\\s+(.*?)(?:/|)>`, 'i'))?.[1] || ''
        for (const a of attrs.matchAll(/([a-z][a-z-]*)(?:="[^"]*"|)/gi)) {
          const prop = a[1]
          if (['class', 'style', ':key', 'v-for', 'v-if', 'v-else', 'v-show', 'ref', 'v-close-popup', 'v-ripple', 'slot'].includes(prop)) continue
...
const allSlugs = [...new Set([...Object.keys(specs), ...Object.keys(docExamples), ...Object.keys(sourceProps)])].sort()
...
for (const slug of allSlugs) {
  ...
  const untestedSourceProps = [...sourcePropSet].filter(p => !testedPropSet.has(p) && !['dark', 'dense', 'disable', 'readonly', 'loading'].includes(p))

  // Check untested doc examples
  const testedLabels = new Set(spec.labels.map(l => l.toLowerCase().replace(/[^a-z0-9]/g, '')))
  const untestedDocs = docs.filter(d => {
    const base = d.file.replace(/\.vue$/, '').toLowerCase().replace(/[^a-z0-9]/g, '')
    return ![...testedLabels].some(label => base.includes(label) || label.includes(base))
  })

  const hasGaps = untestedDocs.length > 0

  if (hasGaps) {
    console.log(`  ~ ${slug} — ${numLabels} variants, ${numConformance} conformance tests`)
    console.log(`    Quasar source: ${numProps} props (${numTestedProps} tested, ${untestedSourceProps.length} uncovered)`)
    console.log(`    Doc examples: ${numDocs} total — ${untestedDocs.length} untested:`)
    for (const d of untestedDocs.slice(0, 6)) {
      console.log(`      ✗ ${d.file} — props: ${d.props.slice(0, 6).join(', ')}${d.props.length > 6 ? '...' : ''}`)
    }
    if (untestedDocs.length > 6) console.log(`      ... and ${untestedDocs.length - 6} more`)
    withGaps++
  } else {
    console.log(`  ✓ ${slug} — ${numLabels} variants, ${numConformance} conformance, ${numDocs} doc examples covered`)
    fullyCovered++
  }
}
...
const gapRanking = Object.entries(specs)
  .map(([slug, s]) => {
    ...
    const untested = docs.filter(d => {
      const base = d.file.replace(/\.vue$/, '').toLowerCase().replace(/[^a-z0-9]/g, '')
      const labels = new Set(s.labels.map(l => l.toLowerCase().replace(/[^a-z0-9]/g, '')))
      return ![...labels].some(label => base.includes(label) || label.includes(base))
    })
    return { slug, labels: s.labels.length, untested: untested.length, total: docs.length }
  })
...
for (const r of gapRanking) {
  console.log(`  ${r.slug}: ${r.labels} variants tested, ${r.untested}/${r.total} doc examples uncovered`)
}
...
console.log('3. Use the props listed from the doc files to construct URL query params')
