...
function parseColor(css) {
  ...
  m = s.match(
    /^rgba?\((\d{1,3}%?)[,\s]+(\d{1,3}%?)[,\s]+(\d{1,3}%?)(?:\s*[,/]\s*([\d.]+%?))?\s*\)$/
  )
  if (m) {
    const to255 = (v) =>
      v.endsWith('%')
...
function checkJSON(filepath) {
  ...
  if (interactive) {
    ...
    if (
      colorsEqual(interactive.backgroundColor, MD3.colors.primary) &&
      interactive.color
    ) {
      if (!colorsEqual(interactive.color, MD3.colors['on-primary'])) {
        ...
        const isIcon =
          ...
          className.includes('q-chip__icon')
        const isOutline =
          className.includes('outline') || className.includes('flat')
        if (isIcon || isOutline) {
          info.passed++
        } else {
          issues.push(
            `  ⚠  Primary bg but text is ${interactive.color} (expected ${MD3.colors['on-primary']}) on <${interactive.tag} class="${interactive.className}">`
          )
          info.failed++
        }
      } else {
...
const results = []
for (const component of readdirSync(styleDir, { withFileTypes: true }).filter(
  (d) => d.isDirectory()
)) {
  const compDir = join(styleDir, component.name)
  const jsonFiles = readdirSync(compDir).filter(
    (f) => f.endsWith('.json') && !f.includes('__style-')
  )
...
console.log(
  '\nVerification:',
  failedChecks === 0 ? '✅ ALL PASS' : '❌ ISSUES FOUND'
)
