...
test.describe('Comprehensive CSS verification', () => {
  test('QBtn md3 light — correct colors and variables', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="component-preview"] .q-btn')
    ).toBeVisible()

    const result = await page.evaluate(() => {
      const el = document.querySelector(
        '[data-testid="component-preview"] .q-btn'
      )!
  ...
  test('QBtn md2 light - md2 radius = --q-radius-sm (4px), same fill color', async ({
    page
  }) => {
  ...
  test('dark mode emits many color tokens (not just primary)', async ({
    page
  }) => {
    await page.goto('/q-btn?style=md3&dark=true', { waitUntil: 'networkidle' })
    ...
    const tokens = await page.evaluate(() => {
      ...
      return keys
        .map((k) => bodyStyle.getPropertyValue(k).trim())
        .filter(Boolean)
    })
  ...
  test('flat button uses primary color for text, not on-primary (white)', async ({
    page
  }) => {
    await page.goto('/q-btn?style=md3&flat=true&label=Flat', {
      waitUntil: 'networkidle'
    })
    await expect(
      page.locator('[data-testid="component-preview"] .q-btn--flat')
    ).toBeVisible()
  ...
  test('QCard md3 — surface container background', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="component-preview"] .q-card')
    ).toBeVisible()

    const result = await page.evaluate(() => {
      const el = document.querySelector(
        '[data-testid="component-preview"] .q-card'
      )!
      const s = getComputedStyle(el)
  ...
  test('no legacy --q-color-primary-bg or --q-color-primary-text in any stylesheet', async ({
    page
  }) => {
    await page.goto('/q-btn?style=md3', { waitUntil: 'networkidle' })
      ...
      for (const sheet of document.styleSheets) {
        try {
          for (const rule of Array.from(sheet.cssRules || [])) {
            ...
            if (
              t.includes('--q-color-primary-bg') ||
              t.includes('--q-color-primary-text')
            )
              return true
          }
        } catch {
